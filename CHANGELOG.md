...
- **Las brochas dejan de llamarse `*Color`, y el marco pasa a las propiedades heredadas de `Control`.** Las cinco propiedades de pintura son `Brush` y no `Color`: el nombre prometía un tipo y la propiedad entregaba otro. En C# `selector.BorderColor = Colors.Red` no compilaba (hay que pasar un `Brush`), y en XAML el type converter tapaba el error, así que el problema sólo aparecía al pasar a código. Peor: escondía que estas propiedades **aceptan cualquier brocha** —un degradado en la barra, una imagen, un `VisualBrush`—, que es de lo más personalizable que tiene el control. Renombres: `FocusBorderColor` → **`FocusBorderBrush`**, `BarFillColor` → **`BarFill`** (sin sufijo, como `Shape.Fill`: cuando la propiedad *es* el relleno, el tipo no se repite en el nombre), `BarDividerColor` → **`BarDividerBrush`**.
- **`BorderColor` y `BorderWidth` se eliminan: el marco usa `Control.BorderBrush` y `Control.BorderThickness`.** Son las propiedades que todo consumidor de WPF ya conoce, así que el control deja de inventarles nombre y se ahorra dos DPs. Lo único que cambia es el valor por defecto, vía `OverrideMetadata`: en `Control` valen `null` y `0` —un control sin marco—, y acá siguen siendo negro y `1` como antes. Esto no contradice la entrada de más abajo que explicaba por qué no era viable llamarla `BorderThickness`: lo inviable era **declarar una propia** con ese nombre (habría ocultado a la heredada con CS0108, y un `TemplateBinding BorderThickness` se habría enlazado igual a la de `Control`, dejando la nueva sin efecto). Usar directamente la heredada convierte esa trampa en el comportamiento deseado. Sin cambios visuales.

- **La medición reserva un margen de seguridad ("yapa") sobre el ancho natural de los textos.** El redondeo de WPF sobre los glifos (`UseLayoutRounding` + mediciones fraccionarias de `FormattedText`) hacía que el ancho "natural" que reporta la plantilla no alcanzara por un pelín al real: el `Caption`/`DetailText` oscilaba entre `CharacterEllipsis` y texto completo según el tamaño de fuente (p. ej. truncaba a 25 y no a 27) aunque el control tuviera hueco de sobra. Ahora `MeasureOverride` redondea el ancho natural hacia arriba y le suma una constante (`MeasureSlack = 3.0`, usada como piso `max(BaseWidth, ceil(natural)+slack)`). Como el clamp a lo disponible sigue intacto, en contenedores angostos la elipsis sigue apareciendo igual; sólo se gana margen cuando hay espacio.
- **Demo — la vista del evento `ValueChanged` pasa a una caja fija** arriba del Master: título "ValueChanged Events:", registro en `Consolas` y ancho de 280 px que lo mantiene alineado con la columna izquierda de los controles.
- **Demo — los pickers de estado (`ShowDetail`, `ValueFollowsDetail`, `ValueBoxDock`, `MouseBehavior`, `InteractionMode`) dejan de ser `CheckBox`/combos** y pasan a `BoundedNumericSelector` con rango 0/1: cada uno muestra su `DetailText` descriptivo (p. ej. `false: (Hide Detail Row)`/`true: (Show Detail Row)`) vía conversores de índice nuevos. El valor por defecto de `ValueBoxDock` sigue siendo `Right` (lo fija la metadata de la DP), y la lista del selector se alineó al eje de la barra (0 = izquierda → `Left`).
- **Demo — ancho de columnas fijo de 280 px** para las tres columnas de controles (dentro del `ScrollViewer`) y reordenamiento: `BaseWidth` bajo `ValueBoxDock`, títulos de cuadro en inglés, `BorderWidth` master en 2 y `FontSize`/`FontFamily` por selector de índice (sin `DataTemplate` de fuentes; se enumeran en runtime).
- **`IsReadOnly` (bool) pasa a `InteractionMode` (`UserInteractionMode`).** El boolean se vuelve un enum con `Interactive` (predeterminado) y `ReadOnly`: el modo de sólo visualización deja de ser una bandera y el enum deja el hueco para futuros modos. La coerción de `Focusable`, la suelta del foco y el bloqueo de mouse/teclado se mantienen exactamente igual con `InteractionMode = UserInteractionMode.ReadOnly`. Sin cambios de comportamiento. (Las entradas anteriores que mencionan `IsReadOnly` refieren al nombre que tenía en ese momento.)
- **`ValueChangeMode` (propiedad y enum) pasan a `MouseBehavior` y `MouseInteractionBehavior`.** El enum se llama como lo que describe (el comportamiento del mouse ante el valor) y la propiedad dice qué comporta la dupla `MouseBehavior = MouseInteractionBehavior`; los valores `ChangeOnClick`/`MustFocusFirst` no cambian. De paso queda reservado el hueco para futuros modos (`ChangeOnHover`, `ChangeOnDoubleClick`, etc.). Sin cambios de comportamiento. (Las entradas anteriores que mencionan `ValueChangeMode` refieren al nombre que tenía en ese momento.)
- **`ValueBoxSide` pasa a llamarse `ValueBoxDock` (propiedad y enum).** El nombre nuevo ancla al concepto `Dock` de WPF (valores `Right`/`Left` intactos) y deja claro que la caja se ancla a un lado de su compañero de fila. Valor, tipo y comportamiento no cambian. (Las entradas anteriores que mencionan `ValueBoxSide` refieren al nombre que tenía en ese momento.)
- **El prefijo `Control` de los bordes desaparece: `ControlBorderColor` → `BorderColor` y `ControlBorderThickness` → `BorderWidth`.** `Control.BorderThickness` (y su DP) ya existen en WPF, así que `BorderThickness` no era viable: la registrada en este control la ocultaria (CS0108) y un `TemplateBinding BorderThickness` enlazaría a la heredada del `Control`, rompiendo los marcos. Queda `BorderWidth` para el grosor (sin colisión) y `BorderColor` para el color, alineados entre sí y con `FocusBorderColor`, `BarFillColor` y `BarDividerColor` (todos en singular). Valor, tipo y comportamiento no cambian. (Las entradas anteriores que mencionan `ControlBorderColor`/`ControlBorderThickness`/`BorderThickness` refieren a los nombres que tenían en ese momento.)
- **`BarBorderColor` pasa a llamarse `BarDividerColor`.** El trazo vertical de 1px (`BorderThickness="0,0,1,0"`) no es un contorno completo de la barra sino un **divisor** entre la porción rellena y la vacía; el nombre nuevo describe la función y no se cruza con `BorderColor`. Valor, tipo (`Brush`) y comportamiento no cambian. (Las entradas anteriores que mencionan `BarBorderColor` refieren al nombre que tenía en ese momento.)
- **El AutoGrow de `Width` pasa a `BaseWidth`: ancho "base" no duro, con encaje interno al contenedor.** Se elimina el enum `StretchMode` (`Fixed`/`AutoGrow`) y los booleanos intermedios `EnableWidthAutoGrow`/`EnableWidthAutoShrink`, y el control **deja de escribir** `FrameworkElement.Width`/`MinWidth` en runtime (eran constraints duras que desbordaban y recortaban los bordes en un contenedor angosto). La nueva propiedad `BaseWidth` (`double`, default `NaN` = automático) es el ancho base/piso desde el que el control crece: en `MeasureOverride` pide `max(BaseWidth, contenido)` pero **nunca más ancho que el hueco que le da el contenedor**, así que los bordes jamás se salen y, cuando el hueco falta, la leyenda y el detalle se truncan con `CharacterEllipsis` en lugar de desbordar. El ancho del número lo reserva el **medidor oculto** de la plantilla (no una propiedad del control).
- **Se elimina el `Viewbox` como patrón de consumo para el encaje interno.** Antes se recomendaba envolver el control del consumidor en `Viewbox Stretch="Uniform" StretchDirection="DownOnly"` para escalarlo en un contenedor angosto; ahora ese comportamiento queda integrado en el control (y la plantilla no envuelve `PART_MainGrid` en un `Viewbox`: eso escalaba la tipografía en vez de truncar con elipsis, que es el comportamiento pedido).
- **`ControlBorderPixels` pasa a llamarse `ControlBorderThickness`** (y luego `BorderWidth`). Es un `Thickness` (no un conteo de píxeles) y WPF mide en DIPs, no en píxeles físicos; además el nombre quedaba desalineado con `ControlBorderColor`. El valor y el comportamiento no cambian. (Las entradas anteriores de este changelog que mencionan `ControlBorderPixels` refieren al nombre que tenía en ese momento.)
...
- **Demo — colores**: cada propiedad que el control maneja como brush (`BarFillColor`, `BarDividerColor`, `BorderColor`, `FocusBorderColor`, `Foreground`, `Background`) pasa a tener su propio selector de color, enganchado directo a esa propiedad (el `CaptionText` dice cuál) y aplicado sobre el Master y sobre el selector mismo. Desaparece el selector de índice previo (`PropertyPicker`) y el `PropertyIndexToNameConverter` que lo acompañaba, porque ya no hace falta elegir "a qué propiedad pintar".
...
- **El control deja de medir texto en C#.** La reserva de ancho del casillero la hacen los **medidores ocultos** de la plantilla (los `TextBlock` con `Minimum` y `Maximum`, ver más arriba), que es la vía *fail-safe*: le pregunta el ancho al mismo motor que después dibuja, en vez de recalcularlo con una fórmula paralela que puede quedar corta. Con eso desaparecen la función pura `TextMeasure.Measure` y su `TextMeasureContext`, que ya no tenían llamador, junto con el `InternalsVisibleTo` que existía sólo para probarlas.
...
- Se eliminan `TitleMode` (`Hidden`/`Framed`/`Frameless`), `ValuePlacement` (`BesideBar`/`OnBar`/`WithTitleFramed`/`WithTitleFrameless`) y `FillForeground`. Sus responsabilidades pasan a tres propiedades independientes, sin valores combinados ni degradaciones:
  ...
  - `ValueBoxDock` (`ValueBoxDock`, default `Right`): lado del casillero del valor respecto de su compañero de fila.
